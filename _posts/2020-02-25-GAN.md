...
$$= -log(4) + 2\cdot JSD(p_{data}||p_g)$$
  ...
  결론적으로 `Theorem1` 이 이야기 하고 싶은 것은  $$p_{data}$$의 distribution 과  $$p_g$$의 distribution에 사이의 차이는 항상 0보다 크거나 같기 때문에 같을 경우에만 `0`을 가지고 그 때 C(G)는 `-log(4)`, 즉 최소값을 가진다. 또한 $$
JSD(p_{data}||p_g)
$$는 항상 0보다 크거나 같기 때문에 C(G)는 `convex`하다고 볼 수 있다. 
