...
* Earth-Mover distance, Wasserstein-1 distance
  ...
  $$W(P_r,P_g)= inf_{γ \in \mathsf{\Pi} (P_r,P_g)}E_{(x,y)∼γ}[ \parallel x−y \parallel ] $$
...
> $$
\mathbb{P}_r
$$ 을 어떤 분포라고 하고 $$\mathbb{P}_\theta $$ 는 밀도 p를 갖는 랜덤 변수 Z에 대한 $$g_\theta(Z)$$의 분포이다. 이 때 $$f: \chi \rightarrow \mathbb{R}$$이 $$\max_{w\in W}\mathbb{E}_{x \sim \mathbb{P}_r}[f_w(x)] - \mathbb{E}_{z \sim p(z)}[f_w(g_\theta(Z))]$$에 대해 해가 존재하며, 그리고 우리는 두 항이 명확하게 정의되었을 때 도함수
$$
\triangledown _ \theta W(P_r, P_ \theta) = −E_{z \sim p(z)} [ \triangledown _ \theta f(g_ \theta(z))]
$$ 를 갖는다. 
